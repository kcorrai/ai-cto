...
} from "@/lib/ai/prompts/performance";
import { findingSchema } from "@/lib/ai/schemas";
import type { RepoBundle } from "@/lib/github/fetcher";

const performanceSchema = z.object({
  score: z.number().int().min(0).max(100).describe("Performance quality score 0–100"),
  findings: z
    .array(findingSchema)
    .describe("Performance issues — only include real, specific findings"),
  strengths: z
    .array(z.string())
    .min(2)
    .max(5)
    .describe("Specific positive performance characteristics (2–5 items)"),
});

export type PerformanceResult = z.infer<typeof performanceSchema>;

export async function runPerformanceModule(bundle: RepoBundle): Promise<{ score: number }> {
  const start = Date.now();

  await db.analysisModule.upsert({
    where: { analysisId_module: { analysisId: bundle.analysisId, module: "performance" } },
    create: { analysisId: bundle.analysisId, module: "performance", status: "running" },
  ...
  try {
    ...
    const { object, inputTokens, outputTokens, durationMs } = await generateAnalysis({
      ...
      prompt: buildPerformanceUserPrompt(bundle),
    });
    ...
    await db.analysisModule.update({
      ...
      data: {
        ...
        rawOutput: {
          ...object,
          findings: validatedFindings,
          promptVariant: getPromptVariant("performance"),
        },
