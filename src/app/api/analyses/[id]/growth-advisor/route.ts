...
import { adviseOnGrowth } from "@/lib/ai/growth-advisor";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ result: meta?.growthAdvisor ?? null });
}
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await adviseOnGrowth({
    ...
    productReadinessScore: (breakdown?.product_readiness as number | undefined) ?? 0,
  ...
  await db.analysis.update({
    ...
    data: { metadata: { ...meta, growthAdvisor: stored } as any },
  });
