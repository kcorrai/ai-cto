...
// Queue consumer (production with Vercel Queues)
const queueHandler = handleCallback(processAnalysis, { visibilityTimeoutSeconds: 600 });

// Direct call handler (fallback when Vercel Queues is not available)
export async function POST(req: Request): Promise<Response> {
  ...
  if (internalSecret && internalSecret === env.ENCRYPTION_KEY) {
    ...
    // Fire and forget — response returns immediately, analysis runs in background
